...
public partial class CommandParser
{
    ...
    private static readonly (string Key, string Help)[] ConfigureHelpEntries =
    {
        ...
        ("QUESTALLPARTY", "Main-quest party-drop convenience. OFF (stock): a main-quest item a monster DROPS on death (gleaming shard, elf-head, the severed heads, iron crown, obsidian talisman, red parchment, spectral webbing, locked wooden box) drops a SINGLE copy to the floor, so a party of N must kill the monster N times — one turn-in each. ON: one copy is placed in EACH engaged player's inventory (over-encumbrance falls to the floor). Only the curated main-quest ground-drop items are affected; normal loot is unchanged. (The other 'received when you kill' quest items — Eternal Fire, Storm Spirit, Heartstone, etc. — already deliver room-wide via the boss's death-spell and are unaffected.)"),
    };
    ...
    private async Task HandleSysopConfigure(string args)
    {
        ...
        if (tokens.Length == 0)
        {
            ...
            await _client.SendLineAsync($"  QUESTALLPARTY: {(_world.QuestDropToAllParty ? "ON" : "OFF")} (main-quest ground-drop items go to each engaged party member, not one to the floor)");
